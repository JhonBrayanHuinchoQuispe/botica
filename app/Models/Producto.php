...
class Producto extends Model
{
    ...
    {
        return $this->belongsTo(Presentacion::class, 'presentacion');
    }
    ...
    {
        return $this->hasMany(ProductLot::class, 'producto_id');
    }

    // Lotes activos (no vencidos, no agotados)
    ...
    public function loteProximoVencer()
    {
        return $this->hasOne(ProductLot::class, 'producto_id')
                    ->where('estado', 'activo')
                    ->where('cantidad_actual', '>', 0)
                    ->where('fecha_vencimiento', '>', now())
