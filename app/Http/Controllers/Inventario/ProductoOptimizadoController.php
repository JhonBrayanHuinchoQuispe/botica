...
class ProductoOptimizadoController extends Controller
{
    ...
    /**
     * Crear nuevo producto con su primer lote
     */
    public function store(ProductoRequest $request): JsonResponse
    {
        try {
            ...
            // Separar datos del producto y del lote
            $datosProducto = [
                'nombre' => $data['nombre'],
                ...
                'stock_actual' => $data['cantidad_inicial'], // Stock inicial = cantidad del primer lote
                'estado' => 'Normal'
            ];
            ...
            if ($request->hasFile('imagen')) {
                $datosProducto['imagen'] = $this->guardarImagen($request->file('imagen'));
            }
            
            // Crear el producto
            $producto = $this->productoRepository->create($datosProducto);
            
            // Crear el primer lote del producto
            $datosLote['producto_id'] = $producto->id;
            $lote = \App\Models\ProductLot::create($datosLote);
            
            // Disparar evento para auditoría
            event(new ProductoActualizado(
                $producto,
                Auth::user(),
                ['nuevo' => $datosProducto, 'primer_lote' => $datosLote],
                'creado'
            ));
            
            DB::commit();
            
            Log::info("Producto creado con lote: {$producto->nombre}", [
            ...
            return response()->json([
                'success' => true,
                'message' => 'Producto y lote creados exitosamente',
            ...
            DB::rollBack();
            Log::error('Error creando producto con lote: ' . $e->getMessage(), [
