...
class ProductoApiController extends Controller
{
    ...
    public function addStock(Request $request, $id)
    {
        try {
            ...
            $product->numero_lote = $request->batch_number;
            $product->fecha_vencimiento = $request->expiry_date;
            $product->save();
    ...
    public function getDetallesConLotes($id)
    {
        try {
            ...
            // Construir URL pública de imagen
            $imagenUrl = asset('assets/images/default-product.svg');
            if (!empty($producto->imagen)) {
                $pathStorage = storage_path('app/public/' . ltrim($producto->imagen, '/'));
                $imagenUrl = file_exists($pathStorage)
