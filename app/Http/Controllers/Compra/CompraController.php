    ...
    public function procesarEntrada(Request $request)
    {
        try {
            ...
            $validatedData = $request->validate([
                ...
                'lote' => 'required|string|max:100|regex:/^[a-zA-Z0-9\-_\s]+$/', // Solo alfanuméricos, guiones y espacios
                'fecha_vencimiento' => 'nullable|date|after_or_equal:today', // No puede ser anterior a hoy
            ...
            if (isset($validatedData['fecha_vencimiento'])) {
                ...
                if ($fechaVencimiento->diffInDays($hoy) < 30) {
                    Log::warning("Producto con vencimiento próximo registrado: {$producto->nombre}, Lote: {$validatedData['lote']}, Vence: {$fechaVencimiento->format('Y-m-d')}");
                }
            }
            ...
            // Actualizar lote y fecha de vencimiento usando datos sanitizados
