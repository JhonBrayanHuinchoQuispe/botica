                            ...
                            @if(isset($presentaciones) && $presentaciones->isNotEmpty())
                                @foreach($presentaciones as $presentacion)
                                    <tr data-id="{{ $presentacion->id }}">
                                        ...
                                        <td data-label="U. Medida" class="col-unidad">
                                            @if(!empty($presentacion->unidad_venta))
                                                <span class="badge-unidad u-{{ $presentacion->unidad_venta }}">{{ $presentacion->unidad_venta }}</span>
                                            @endif
                                        </td>
                                        <td data-label="Contenido">{{ $presentacion->factor_unidad_base ?? '' }}</td>
                                        <td data-label="Precio" class="col-precios">
                                            @php
                                                $precioPresentacion = isset($presentacion->precio_venta) ? (float) $presentacion->precio_venta : null;
                                                $factor = isset($presentacion->factor_unidad_base) ? max(1, (int) $presentacion->factor_unidad_base) : 1;
                                                $precioUnidadBase = ($precioPresentacion && $factor) ? ($precioPresentacion / $factor) : null;
                                            @endphp
                                            <div class="precios-stack">
                                                <div class="precio-item precio-presentacion">
                                                    <span class="precio-label">P. Presentación:</span>
                                                    <span class="precio-valor">{{ $precioPresentacion !== null ? 'S/ ' . number_format($precioPresentacion, 2) : '—' }}</span>
                                                </div>
                                                <div class="precio-item precio-base">
                                                    <span class="precio-label">P. Unidad base:</span>
                                                    <span class="precio-valor">{{ $precioUnidadBase !== null ? 'S/ ' . number_format($precioUnidadBase, 2) : '—' }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td data-label="Stock (presentación)">
                                            @if(isset($presentacion->factor_unidad_base) && isset($presentacion->stock_unidades_base))
                                                {{ floor($presentacion->stock_unidades_base / max(1, $presentacion->factor_unidad_base)) }}
                                            @endif
                                        </td>
                                        ...
                                        <td data-label="Acciones">
                                            ...
                                            <label class="toggle-switch" title="Activar/Desactivar">
                                                <input type="checkbox" class="estado-toggle" data-id="{{ $presentacion->id }}" {{ $presentacion->activo ? 'checked' : '' }}>
    ...
    <div class="modal-presentacion-container">
        ...
        <form id="formAgregarPresentacion" class="modal-presentacion-form">
            ...
            <div class="presentacion-grid">
                <!-- Fila 1: Unidad y Factor -->
                <div class="form-row">
                    <div class="form-field flex-2">
                        <label for="agregarUnidadVenta">Unidad de venta</label>
                        <select id="agregarUnidadVenta" name="unidad_venta" class="input-modal-presentacion">
                    ...
                    <div class="form-field flex-1">
                        <label for="agregarFactor">Contenido</label>
                        <input type="number" id="agregarFactor" name="factor_unidad_base" class="input-modal-presentacion" min="1" step="1" placeholder="10" required>
                        <small class="hint">Unidades base</small>
                    </div>
                </div>

                <!-- Fila 2: Precio y Toggle de fraccionamiento -->
                <div class="form-row">
                    <div class="form-field flex-2">
                        <label for="agregarPrecioVenta">Precio por esta unidad</label>
                        <input type="number" id="agregarPrecioVenta" name="precio_venta" class="input-modal-presentacion" min="0" step="0.01" placeholder="4.50">
                    </div>
                    <div class="form-field flex-1 toggle-field">
                        <label>Unidades sueltas</label>
                        <label class="toggle-switch" title="Permite vender la unidad base">
                            <input type="checkbox" id="agregarFraccionable" name="permite_fraccionamiento">
                            <span class="toggle-slider"></span>
                        </label>
                    </div>
                </div>

                <!-- Fila 3: Cálculo automático -->
                <div class="form-row auto-calc-section">
                    <div class="form-field flex-1 toggle-field">
                        <label>Cálculo automático</label>
                        <label class="toggle-switch" title="Calcular desde precio de unidad base y factor">
                            <input type="checkbox" id="agregarCalcularAuto" name="calcular_automatico">
                            <span class="toggle-slider"></span>
                        </label>
                    </div>
                    <div class="form-field flex-2">
                        <label for="agregarPrecioBase">Precio por unidad base</label>
                        <input type="number" id="agregarPrecioBase" class="input-modal-presentacion" min="0" step="0.01" placeholder="0.50">
                    </div>
                </div>
    ...
    <div class="modal-presentacion-container">
        ...
        <form id="formEditarPresentacion" class="modal-presentacion-form">
            ...
            <div class="presentacion-grid">
                ...
                <div class="form-row">
                    <div class="form-field flex-2">
                        <label for="editarUnidadVenta">Unidad de venta</label>
                        <select id="editarUnidadVenta" name="unidad_venta" class="input-modal-presentacion">
                            <option value="unidad">Unidad</option>
                            <option value="blister">Blíster</option>
                            <option value="caja">Caja</option>
                            <option value="frasco">Frasco</option>
                    ...
                    <div class="form-field flex-1">
                        <label for="editarFactor">Contenido</label>
                        <input type="number" id="editarFactor" name="factor_unidad_base" class="input-modal-presentacion" min="1" step="1" placeholder="10" required>
                        <small class="hint">Unidades base</small>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-field flex-2">
                        <label for="editarPrecioVenta">Precio por esta unidad</label>
                        <input type="number" id="editarPrecioVenta" name="precio_venta" class="input-modal-presentacion" min="0" step="0.01" placeholder="4.50">
                    </div>
                    <div class="form-field flex-1 toggle-field">
                        <label>Unidades sueltas</label>
                        <label class="toggle-switch" title="Permite vender la unidad base">
                            <input type="checkbox" id="editarFraccionable" name="permite_fraccionamiento">
                            <span class="toggle-slider"></span>
                        </label>
                    </div>
                </div>
                ...
                <div class="form-row auto-calc-section">
                    <div class="form-field flex-1 toggle-field">
                        <label>Cálculo automático</label>
                        <label class="toggle-switch" title="Calcular desde precio de unidad base y factor">
                            <input type="checkbox" id="editarCalcularAuto" name="calcular_automatico">
                            <span class="toggle-slider"></span>
                        </label>
                    </div>
                    <div class="form-field flex-2">
                        <label for="editarPrecioBase">Precio por unidad base</label>
                        <input type="number" id="editarPrecioBase" class="input-modal-presentacion" min="0" step="0.01" placeholder="0.50">
                    </div>
                </div>
...
.skeleton-row { display:grid; grid-template-columns: 64px 1fr 2fr 120px 110px 120px 140px 170px; gap: 16px; align-items:center; padding: 14px 10px; border-bottom: 1px solid #f1f5f9; }
