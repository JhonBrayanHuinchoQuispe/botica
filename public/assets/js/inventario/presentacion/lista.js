...
function renderPresentacionesTabla() {
  ...
  paginados.forEach(item => {
    ...
    const unidadChip = item.unidad_venta ? `<span class="chip"><iconify-icon icon="mdi:package-variant"></iconify-icon>${item.unidad_venta}</span>` : '';
    const factorChip = item.factor_unidad_base ? `<span class="chip"><iconify-icon icon="mdi:numeric"></iconify-icon>factor ${item.factor_unidad_base}</span>` : '';
    const precioPresentacion = (item.precio_venta || item.precio_venta === 0) ? Number(item.precio_venta) : null;
    const factor = Number(item.factor_unidad_base) || 1;
    const precioUnidadBase = (precioPresentacion !== null && factor > 0) ? (precioPresentacion / Math.max(1, factor)) : null;
    const stockPresentacion = (Number.isFinite(item.factor_unidad_base) && Number.isFinite(item.stock_unidades_base))
      ? Math.floor(item.stock_unidades_base / Math.max(1, item.factor_unidad_base))
      : '';
    tr.innerHTML = `
      ...
      <td data-label="U. Medida" class="col-unidad">${item.unidad_venta ? `<span class="badge-unidad u-${item.unidad_venta}">${item.unidad_venta}</span>` : ''}</td>
      <td data-label="Contenido">${item.factor_unidad_base || ''}</td>
      <td data-label="Precio" class="col-precios">
        <div class="precios-stack">
          <div class="precio-item precio-presentacion">
            ...
            <span class="precio-valor">${precioPresentacion !== null ? `S/ ${precioPresentacion.toFixed(2)}` : '—'}</span>
          </div>
          <div class="precio-item precio-base">
            <span class="precio-label">P. Unidad base:</span>
            <span class="precio-valor">${precioUnidadBase !== null ? `S/ ${precioUnidadBase.toFixed(2)}` : '—'}</span>
          </div>
        </div>
      </td>
      <td data-label="Stock (presentación)">${stockPresentacion}</td>
