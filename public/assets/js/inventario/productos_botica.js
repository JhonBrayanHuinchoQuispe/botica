...
async function cargarProveedores(provSel=null) {
  try {
    const rp = await fetch('/api/compras/buscar-proveedores');
    const dp = await rp.json();
    ...
    if (dp.success) {
      const opts = ['<option value="">Seleccionar</option>']
        .concat(dp.data.map(pr => {
          const display = pr.nombre || pr.nombre_comercial || pr.ruc || (`Proveedor #${pr.id}`);
...
async function abrirModalEdicion(productId) {
  try {
    ...
    if (typeof window.abrirModalEditarProducto === 'function') {
      window.abrirModalEditarProducto({
        id: p.id,
        nombre: p.nombre,
        categoria: p.categoria,
        marca: p.marca,
