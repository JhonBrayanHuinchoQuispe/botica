...
class ValidacionesTiempoReal {
    ...
    setupEventListeners() {
        ...
        this.setupModalValidations('modalAgregar', 'formAgregarProducto', {
            ...
            'numero_lote': this.validateNumeroLote.bind(this),
            'codigo_barras': this.validateCodigoBarras.bind(this),
            'cantidad_inicial': this.validateStock.bind(this),
            ...
            'fabricante': this.validateFabricante.bind(this),
            'pais_origen': this.validatePaisOrigen.bind(this)
        });
        ...
        this.setupModalValidations('modalEditar', 'formEditarProducto', {
            ...
            'edit-numero_lote': this.validateNumeroLote.bind(this),
            'edit-codigo_barras': this.validateCodigoBarras.bind(this),
            'edit-cantidad_inicial': this.validateStock.bind(this),
            ...
            'edit-fabricante': this.validateFabricante.bind(this),
            'edit-pais_origen': this.validatePaisOrigen.bind(this)
        });
    }
    ...
    validateNumeroLote(field) {
        const value = field.value.trim();
        
        this.clearFieldError(field);

        if (!value) {
            this.showFieldError(field, 'El número de lote es obligatorio');
            return false;
        }

        // Validación de formato del lote (más estricto para farmacia)
        const loteRegex = /^[A-Z0-9\-_]+$/;
        if (!loteRegex.test(value)) {
            this.showFieldError(field, 'El lote solo puede contener letras mayúsculas, números, guiones y guiones bajos');
            return false;
        }

        if (value.length < 4) {
            this.showFieldError(field, 'El número de lote debe tener al menos 4 caracteres');
            return false;
        }

        if (value.length > 30) {
            this.showFieldError(field, 'El número de lote no puede tener más de 30 caracteres');
            return false;
        }
    ...
    validateFechaVencimiento(field) {
        ...
        if (!value) {
            this.showFieldError(field, 'La fecha de vencimiento es obligatoria');
            return false;
        }
        ...
        if (fecha <= hoy) {
            this.showFieldError(field, 'La fecha de vencimiento debe ser futura');
            return false;
        }
    ...
    validateTemperatura(field) {
        ...
        // Validar formato de temperatura (número + °C)
        const tempRegex = /^-?\d+(\.\d+)?\s*°?C?$/i;
        if (!tempRegex.test(value)) {
            this.showFieldError(field, 'Formato inválido. Ejemplos: 25°C, -5°C, 2-8°C');
            return false;
        }
    ...
    validateRegistroSanitario(field) {
        const value = field.value.trim();
        ...
        // Validar formato de registro sanitario
        const registroRegex = /^[A-Z0-9\-\/]+$/;
        if (!registroRegex.test(value)) {
            this.showFieldError(field, 'Solo se permiten letras mayúsculas, números, guiones y barras');
            return false;
        }

        if (value.length < 5) {
            this.showFieldError(field, 'El registro sanitario debe tener al menos 5 caracteres');
            return false;
        }

        if (value.length > 50) {
            this.showFieldError(field, 'El registro sanitario no puede tener más de 50 caracteres');
            return false;
        }
    ...
    validateFabricante(field) {
        ...
        if (value.length < 2) {
            this.showFieldError(field, 'El fabricante debe tener al menos 2 caracteres');
            return false;
        }

        if (value.length > 100) {
            this.showFieldError(field, 'El fabricante no puede tener más de 100 caracteres');
            return false;
        }
        ...
        if (soloNumerosRegex.test(value)) {
            this.showFieldError(field, 'El fabricante no puede contener solo números');
            return false;
        }
    ...
    validatePaisOrigen(field) {
        ...
        if (value.length < 2) {
            this.showFieldError(field, 'El país de origen debe tener al menos 2 caracteres');
            return false;
        }

        if (value.length > 50) {
            this.showFieldError(field, 'El país de origen no puede tener más de 50 caracteres');
            return false;
        }

        // Validar que no contenga números
        const contieneNumerosRegex = /\d/;
        if (contieneNumerosRegex.test(value)) {
            this.showFieldError(field, 'El país de origen no puede contener números');
            return false;
        }
    ...
    async validateForm(formId) {
        ...
        const validations = [
            ...
            { field: form.querySelector(isEdit ? '#edit-numero_lote' : '[name="numero_lote"]'), validator: this.validateNumeroLote.bind(this) },
