document.addEventListener('DOMContentLoaded', function() {
    ...
    function validarCampo(input) {
        ...
        switch(nombre) {
            ...
            case 'stock_minimo':
                ...
                break;
            case 'temperatura_almacenamiento':
            case 'registro_sanitario':
            case 'pais_origen':
            case 'observaciones_lote':
                // Campos opcionales
                esValido = true;
                break;
            case 'precio_compra':
    ...
    function validarFormulario() {
        ...
        const numeroLote = formAgregarProducto.querySelector('input[name="numero_lote"]').value.trim();
        const codigoBarras = formAgregarProducto.querySelector('input[name="codigo_barras"]').value.trim();
        const cantidadInicial = formAgregarProducto.querySelector('input[name="cantidad_inicial"]').value.trim();
        ...
        if (cantidadInicial && (isNaN(parseInt(cantidadInicial)) || parseInt(cantidadInicial) <= 0)) {
            formularioValido = false;
        }
