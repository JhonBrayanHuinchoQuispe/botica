...
class POSProfesional {
    ...
    validarProcesamientoVenta() {
        ...
        let puedeProesar = false;

        // Verificar que hay productos en el carrito
        if (this.carrito.length > 0) {
            // Para efectivo: verificar que hay suficiente dinero
            if (this.metodosPagoSeleccionado === 'efectivo') {
                const efectivoInput = document.getElementById('efectivoRecibido');
                const efectivoRecibido = efectivoInput ? parseFloat(efectivoInput.value) || 0 : 0;
                const total = this.calcularTotal();
                
                puedeProesar = efectivoRecibido >= total;
            } else {
                // Para tarjeta y yape: solo verificar que hay productos
                puedeProesar = true;
            }
        }

        // Actualizar estado del botón
        btnProcesar.disabled = !puedeProesar;
        ...
        console.log(`🔄 Validación procesamiento: ${puedeProesar ? 'PUEDE' : 'NO PUEDE'} procesar (${this.metodosPagoSeleccionado})`);
    }
    ...
    async cargarProductosMasVendidos() {
        try {
            ...
            this.actualizarTituloProductos('Productos Más Vendidos');
    ...
    crearTarjetaProducto(producto) {
        ...
        // Agregar event listener solo si hay stock
    ...
    cambiarCantidad(index, nuevaCantidad) {
        ...
        if (nuevaCantidad > item.stock_disponible) {
            this.mostrarError('Cantidad supera el stock disponible');
            return;
        }
    ...
    async procesarVenta() {
        ...
        // Obtener tipo de comprobante de forma segura
        const comprobanteElement = document.getElementById('conComprobante');
        const conComprobante = comprobanteElement && comprobanteElement.checked;
        const tipoComprobante = conComprobante ? 'boleta' : 'ticket';
        ...
        console.log('📋 Procesando venta...', { metodo: this.metodosPagoSeleccionado, comprobante: conComprobante });
        
        // Loading optimizado
        const btnProcesar = document.getElementById('btnProcesarVenta');
        if (!btnProcesar) {
            this.mostrarError('Botón de procesamiento no encontrado');
            return;
        }
        ...
        try {
            ...
            const response = await fetch('/api/punto-venta/procesar-venta', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (data.success) {
                ...
                // Mostrar SweetAlert según tipo de comprobante
                if (conComprobante) {
                    // CON comprobante: mostrar opciones Ticket/PDF
                    this.mostrarExitoConComprobante(venta);
                } else {
                    // SIN comprobante: mensaje simple que desaparece automáticamente
                    this.mostrarExitoSinComprobante(venta);
                }
    ...
    mostrarExitoConComprobante(venta) {
        Swal.fire({
            ...
            `,
            showDenyButton: true,
            confirmButtonText: '<i class="fas fa-receipt"></i> Generar Ticket',
            denyButtonText: '<i class="fas fa-file-pdf"></i> Generar PDF',
    ...
    generarPDF(venta) {
        Swal.fire({
            ...
            html: `
                <div style="text-align: center; padding: 20px;">
                    <div style="font-size: 3em; margin-bottom: 16px;">📄</div>
                    <p>Generando PDF para la venta ${venta.numero_venta}...</p>
                </div>
            `,
            showConfirmButton: false,
            timer: 2000
        }).then(() => {
            // Aquí implementarás la lógica real del PDF
            console.log('📄 Generando PDF para venta:', venta.numero_venta);
            Swal.fire({
                icon: 'success',
                title: 'PDF Generado',
    ...
    actualizarEstadisticas(stats) {
        document.getElementById('ventasHoy').textContent = stats.ventas || '0';
        document.getElementById('totalHoy').textContent = `S/. ${parseFloat(stats.total || 0).toFixed(2)}`;
    }
