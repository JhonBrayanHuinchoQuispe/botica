...
async function consultarDni() {
    ...
    try {
        ...
        if (data.success && data.cliente) {
            ...
            // Mostrar fuente de la consulta si está disponible
            const fuenteElement = document.getElementById('fuenteConsulta');
            if (fuenteElement) {
                if (data.message && data.message.includes('RENIEC')) {
                    fuenteElement.textContent = 'Consultado en RENIEC';
                    fuenteElement.style.color = '#10b981';
                } else {
                    fuenteElement.textContent = 'Desde base de datos local';
                    fuenteElement.style.color = '#6b7280';
                }
            }
