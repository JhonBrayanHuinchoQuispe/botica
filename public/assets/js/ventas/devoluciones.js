    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            Swal.fire({
                ...
                actualizarInterfazDespuesDevolucion();
                limpiarSeleccion();
            });
        } else {
            // Error del servidor
 ...
 */
function actualizarInterfazDespuesDevolucion() {
    // Limpiar formulario de búsqueda
    const codigoInput = document.getElementById('codigo_venta');
    if (codigoInput) {
        codigoInput.value = '';
    }
    
    // Ocultar sección de productos si existe
    const productosSection = document.querySelector('.productos-devolucion');
    if (productosSection) {
        productosSection.style.display = 'none';
    }
    
    // Limpiar tabla de productos
    const tbody = document.querySelector('#productos-devolucion tbody');
    if (tbody) {
        tbody.innerHTML = '';
    }
    ...
    const procesarBtn = document.getElementById('procesar-devolucion');
    if (procesarBtn) {
        procesarBtn.disabled = true;
        procesarBtn.classList.add('disabled');
    }
    
    // Mostrar mensaje de éxito en la interfaz
    const alertContainer = document.querySelector('.alert-container') || document.querySelector('.main-content');
    if (alertContainer) {
    ...
    console.log('✅ Interfaz actualizada después de devolución');
}
